/**
 * 🚀 TokiSync - Module-Safe Integrated Fix
 */
...
window.getDynamicLink = function(series) {
    ...
    var domainNum = (cat === "Novel") ? saved.booktoki : (cat === "Manga" ? saved.manatoki : saved.newtoki);
    var baseUrl = (cat === "Novel") ? "booktoki" + domainNum + ".com/novel/" : 
                 (cat === "Manga") ? "manatoki" + domainNum + ".net/comic/" : 
                 "newtoki" + domainNum + ".com/webtoon/";
    return "https://" + baseUrl + (series.sourceId || "");
};

// [2. 목록 열기 핸들러] - index.js의 함수를 호출
window.handleOpenEpisodes = function(id, name, index) {
    ...
    if (typeof window.openEpisodeList === 'function') {
        ...
        console.warn("openEpisodeList 모듈 로딩 대기 중...");
        setTimeout(() => {
            if (window.openEpisodeList) window.openEpisodeList(id, name, index);
            else alert("목록 모듈을 불러올 수 없습니다. index.js에서 window.openEpisodeList 할당 여부를 확인하세요.");
        }, 500);
    }
};
...
window.renderRecentList = async function() {
    try {
        ...
        console.warn("북마크 로드 실패: GAS Deployment ID를 확인하세요.");
    }
};
